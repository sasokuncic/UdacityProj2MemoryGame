...
// Note: Code from http://stackoverflow.com/a/2450976
...
function cardDisplay(cardSelected) {
 ...
 // @description Checks player's move
function cardCheck() {
    ...
    if (cardsOpened.length == 2) {
        ...
        if (cardsOpened[0].type == cardsOpened[1].type ) {
            ...
            if (cards.length == (cardsMatchedNum * 2)) {
                // console.log ('All cards matched!'); // DBG
                clearInterval(interval);
                openModalWindow();
            }
        } else {
...
// @description: Cards matching manipulation
// Note: for the animation of matched and not matched cards used Daniel's Eden, Animate CSS, 
// Note: used animation library described at https://github.com/daneden/animate.css/blob/master/README.md 
...
// @description: Manipulation of cards which not match
...
// @description: Congratulations popup - modal window 
...
// @description: Play Again button in modal window clicked
function playAgain() {
    modal.style.display = "none";
